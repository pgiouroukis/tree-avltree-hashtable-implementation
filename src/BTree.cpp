...
{ //initialize a node by setting the proper attributes
    Node *node = new Node;
    node->word = w;
    node->occurrences = 1;
...
Node *BTree::addWord(string w)
{
    ...
    else
    {
        ...
        while (true)
        {
            ...
            { //if the word we are trying to add is already in the tree...
            ...
            { //if the word we are trying to add is "smaller" than the current word we are at...
            ...
            { //if the word we are trying to add is "bigger" than the current word we are at...
...
bool BTree::deleteWord(string w)
{
    ...
    else
    {
        ...
        else
        {
            ...
            { // the latter does not apply in the special case that tptr is right child of ptr
...
void inOrder(Node *n)
{ //recursive inOrder traversal, starting from node n
...
{ //recursive postOrder traversal, starting from node n
...
{ //start postOrder traversal from the root of the tree
...
{ //recursive preOrder traversal, starting from node n
...
{ //start preOrder traversal from the root of the tree
...
{ // update the heights of parent nodes, starting from "node" all the way to the root
